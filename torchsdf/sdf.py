...
def compute_sdf(pointclouds, face_vertices, face_vert_normals):
    return _UnbatchedTriangleDistanceCuda.apply(pointclouds, face_vertices, face_vert_normals)


class _UnbatchedTriangleDistanceCuda(torch.autograd.Function):
    @staticmethod
    def forward(ctx, points, face_vertices, face_vert_normals):
        ...
        _C.unbatched_triangle_distance_forward_cuda(
            points, face_vertices, face_vert_normals, min_dist, dist_sign, normals, clst_points)
    ...
    def backward(ctx, grad_dist, grad_dist_sign, grad_normals, grad_clst_points):
        ...
        grad_face_vert_normals = None
        _C.unbatched_triangle_distance_backward_cuda(
            grad_dist, points, clst_points, grad_points)
        return grad_points, grad_face_vertices, grad_face_vert_normals
